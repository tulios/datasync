...
public class TesteTblcaracterizacaofamiliaaluno extends TesteBarco {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliaaluno t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliaaluno t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliaaluno t").getSingleResult();
