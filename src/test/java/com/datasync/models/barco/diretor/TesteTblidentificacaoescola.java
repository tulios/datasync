...
public class TesteTblidentificacaoescola extends TesteBarco {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblidentificacaoescola t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblidentificacaoescola t").getSingleResult();
        ...
        Tblidentificacaoescola var = new Tblidentificacaoescola();
        var.setIdformulario("1");
        var.setQuantidadecomunidade(1);
        var.setQuaiscomunidades("comunidades");
        var.setQuantidadealuno(1);
        var.setQuantidadealunobarco(1);
        var.setQuantidaderota(1);
        getLocalEm().persist(var);

        local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblidentificacaoescola t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblidentificacaoescola t").getSingleResult();
