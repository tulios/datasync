...
public class TesteTblcaracterizacaofamiliapaisauxilio extends TesteBarco {
	...
	public void verificaSincronismo() throws Exception{
		...
		Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliapaisauxilio t").getSingleResult();
		assertEquals(0, local.intValue());

		Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliapaisauxilio t").getSingleResult();
		...
		Tblcaracterizacaofamiliapaisauxilio var = new Tblcaracterizacaofamiliapaisauxilio();
		var.setIdtipoauxilio(1);
		var.setIdformulario("1");
		var.setQualoutro("Outro");
		getLocalEm().persist(var);

		local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliapaisauxilio t").getSingleResult();
		...
		open();
		server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblcaracterizacaofamiliapaisauxilio t").getSingleResult();
