...
public class TesteTblinformacaopaissugestao extends TesteBarco {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblinformacaopaissugestao t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblinformacaopaissugestao t").getSingleResult();
        ...
        Tblinformacaopaissugestao var = new Tblinformacaopaissugestao();
        var.setIdtiposugestao(1);
        var.setIdformulario("1");
        var.setQualoutro("1");
        getLocalEm().persist(var);

        local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblinformacaopaissugestao t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblinformacaopaissugestao t").getSingleResult();
