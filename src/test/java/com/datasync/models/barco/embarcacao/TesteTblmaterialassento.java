...
public class TesteTblmaterialassento extends TesteBarco {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblmaterialassento t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblmaterialassento t").getSingleResult();
        ...
        Tblmaterialassento var = new Tblmaterialassento();
        var.setIdtipoassento(1);
        var.setIdformulario("1");
        var.setQuaisoutros("quais");
        getLocalEm().persist(var);

        local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblmaterialassento t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblmaterialassento t").getSingleResult();
