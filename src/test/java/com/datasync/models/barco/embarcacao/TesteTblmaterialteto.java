...
public class TesteTblmaterialteto extends TesteBarco {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblmaterialteto t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblmaterialteto t").getSingleResult();
        ...
        Tblmaterialteto var = new Tblmaterialteto();
        var.setIdtipoteto(1);
        var.setIdformulario("1");
        var.setQuaisoutros("quais");
        getLocalEm().persist(var);

        local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblmaterialteto t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblmaterialteto t").getSingleResult();
