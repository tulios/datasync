...
public class TesteTbldeslocamento extends TesteBicicleta {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tbldeslocamento t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tbldeslocamento t").getSingleResult();
        ...
        Tbldeslocamento var = new Tbldeslocamento();
        var.setIdformulario("1");
        var.setIdtipoatividade(1);
        var.setQuaisoutras("1");
        getLocalEm().persist(var);

        local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tbldeslocamento t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tbldeslocamento t").getSingleResult();
