...
public class TesteTblprofessorguardabicicleta extends TesteBicicleta {
    ...
    public void verificaSincronismo() throws Exception{
        ...
        indexables.add(new Tblprofessorguardabicicleta());

        Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblprofessorguardabicicleta t").getSingleResult();
        assertEquals(0, local.intValue());

        Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblprofessorguardabicicleta t").getSingleResult();
        ...
        Tblprofessorguardabicicleta var = new Tblprofessorguardabicicleta();
        var.setIdformulario("1");
        var.setIdtipoondeguardabicicleta(1);
        var.setQuaisoutros("1");
        getLocalEm().persist(var);

        local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblprofessorguardabicicleta t").getSingleResult();
        ...
        open();
        server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblprofessorguardabicicleta t").getSingleResult();
