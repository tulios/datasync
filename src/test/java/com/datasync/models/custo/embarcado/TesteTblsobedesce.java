...
public class TesteTblsobedesce extends TesteCusto {
	...
	public void verificaSincronismo() throws Exception{
		...
		Number local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblsobedesce t").getSingleResult();
		assertEquals(0, local.intValue());

		Number server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblsobedesce t").getSingleResult();
		...
		Tblsobedesce var = new Tblsobedesce();
		var.setCodigoponto(1);
		var.setIdformulario("1");
		var.setQuantidadealunosobe(1);
		var.setQuantidadecaronasobe(1);
		var.setQuantidadealunodesce(1);
		var.setQuantidadecaronasdesce(1);
		getLocalEm().persist(var);

		local = (Number) getLocalEm().createQuery("select count(t.idformulario) from Tblsobedesce t").getSingleResult();
		...
		open();
		server = (Number) getServerEm().createQuery("select count(t.idformulario) from Tblsobedesce t").getSingleResult();
